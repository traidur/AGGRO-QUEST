...
once declarations exist, but nothing decided *whether* an AI hero should risk declaring a Node
that might turn out contested (blind redraw, unpreviewed mob) versus a different Node in the
same Zone.

**Resolved same day, by an observation about the simulator specifically (not a tabletop rule
change):** `advance_board` only ever reads the final `pending_declarations` dict once every
hero has an entry — it has no way to know, and doesn't care, *how* each entry got computed. So
the driver is free to compute each AI hero's declaration in priority-token order, each one
reading what higher-priority heroes have *already submitted* this round, and only call
`advance_board` once at the very end. This collapses the whole opponent-modeling problem: "will
I get blind-refilled if I declare this Node?" stops being a probability estimate about unknown
intentions and becomes a known fact (has a higher-priority hero already claimed it this round),
answerable with the same cost%/win% diagnostics the solo AI already has — known visible mob at
an uncontested Node vs. a computable expected blind-redraw outcome at a contested one.

**The one real tradeoff, named rather than hidden:** this makes the simulated AI strictly more
informed than a real, blind-declaring player would be at the actual table (Move-and-declare is
genuinely simultaneous there). Accepted as the right choice for this simulator specifically —
its job is balance/pacing validation, not modeling imperfect-information dynamics as its own
research question, and a real player guessing worse would only make outcomes murkier to
validate against, not cleaner. **Folded into task #65** (drive an actual competitive game
through the barrier) as the AI's own declare-order logic, rather than kept as a separate task
(the old task #66) — there's no remaining design question that needs settling before building
it, just an implementation detail of that same driver loop.
