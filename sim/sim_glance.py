...
def warrior_resolve_round_unlocked(state, card_name, stance, round_num, mob_pattern, hero_max_hp, hero_hp, mob_hp_remaining, mob_hp_total):
    ...
    return RoundOutcome(new_hp=new_hp, new_mob_hp_remaining=mob_hp_remaining-dmg_dealt, new_hero_max_hp=hero_max_hp,
...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    for rnd in range(3):
        ...
        # RULE: GLANCING BLOWS (HALF DAMAGE ROUNDED DOWN INSTEAD OF 0)
        if evades_A and card_data_B.get("combat_type", "melee") == "melee": 
            raw_dmg_B = math.floor(raw_dmg_B / 2)
        if evades_B and card_data_A.get("combat_type", "melee") == "melee": 
            raw_dmg_A = math.floor(raw_dmg_A / 2)
...
totals = {c: {"W": 0, "L": 0, "T": 0} for c in class_names}
cache = {}
    ...
    for cB in class_names:
        ...
        for sA_list in seqs_A:
            for sB_list in seqs_B:
                ...
                if ev > 0.05: totals[cA]["W"] += 1
                elif ev < -0.05: totals[cA]["L"] += 1
                else: totals[cA]["T"] += 1

print("--- ACTUAL HAND-VS-HAND WINS (GLANCING BLOWS + UNLOCKED EXECUTE, NO SCOREBOARD BUFFS) ---")
sorted_wl = sorted(totals.items(), key=lambda item: item[1]['W'], reverse=True)
for c, wl in sorted_wl:
    print(f"{c:12}: {wl['W']} Wins, {wl['L']} Losses, {wl['T']} Ties")
