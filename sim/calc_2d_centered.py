...
def eval_assignment(mod_matrix):
    ...
    wl_score = sum(abs(wl[c] - 4) for c in classes)
    
    # Tiny penalty for large absolute numbers to force it to center around 0
    mag_penalty = 0.001 * sum(abs(mod_matrix[c][a]) for c in classes for a in archetypes)
    return wl_score + mag_penalty
...
for _ in range(1000):
    ...
    if curr_score < best_score:
        ...
        best_mod = current
        if int(best_score) == 0 and best_score < 1.0: # Keep searching for smaller magnitudes even if W/L is perfect
            pass
...
for cA in classes:
    for cB in classes:
        ...
        archB = ARCH_MAP[cB]
        archA = ARCH_MAP[cA]
        ev = MATRIX[cA][cB] + best_mod[cA][archB] - best_mod[cB][archA]
        if ev > 0: wl[cA] += 1
