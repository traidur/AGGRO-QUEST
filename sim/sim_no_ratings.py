...
for cA in ["Warrior"]:
    ...
    all_seqs_A = []
    for h in hands_A: all_seqs_A.extend(get_sequences(cA, h))
        
    for cB in ["Wizard"]:
        hands_B = CLASSES[cB][3]
        all_seqs_B = []
        for h in hands_B: all_seqs_B.extend(get_sequences(cB, h))
        ...
        for _ in range(ITERATIONS):
            ...
            # NO STATIC MODIFIERS!
            score_A = dmgA + tokens_A
            score_B = dmgB + tokens_B
        ...
        avg = np.mean(history_A)
        wr = (wins / ITERATIONS) * 100
        print(f"{cA} vs {cB} (NO RATINGS):")
        print(f"Win Rate: {wr:.1f}%")
        print(f"Average Tokens Held by Warrior: {avg:.1f}")
        print(f"99th Percentile Max Tokens: {int(p99)}")
