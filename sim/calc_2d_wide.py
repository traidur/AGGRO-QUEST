...
for _ in range(500):
    current = {c: {a: random.choice([0, 1, 2, 3, 4, 5]) for a in archetypes} for c in classes}
        ...
        for c in classes:
            for a in archetypes:
                for step in [-1, 1]:
                    ...
                    neighbor[c][a] += step
                    if neighbor[c][a] < 0 or neighbor[c][a] > 5: continue
...
for cA in classes:
    for cB in classes:
        ...
        if ev > 0: wl[cA] += 1
        if cA == 'Warrior' and cB == 'Wizard': print(f"Warrior vs Wizard EV: {ev:+.2f}")
for c in classes:
