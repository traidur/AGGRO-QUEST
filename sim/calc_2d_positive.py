...
for _ in range(500):
    current = {c: {a: random.choice([0, 1, 2, 3]) for a in archetypes} for c in classes}
        ...
        for c in classes:
            for a in archetypes:
                for step in [-1, 1]:
                    ...
                    neighbor[c][a] += step
                    if neighbor[c][a] < 0 or neighbor[c][a] > 3: continue
...
for cA in classes:
    for cB in classes:
        ...
        archB = ARCH_MAP[cB]
        archA = ARCH_MAP[cA]
        if MATRIX[cA][cB] + best_mod[cA][archB] - best_mod[cB][archA] > 0:
            wl[cA] += 1
for c in classes:
    print(f"{c:12}: {wl[c]} W - {8 - wl[c]} L")
    print(f"   vs Martial: +{best_mod[c]['Martial']}")
    print(f"   vs Magic  : +{best_mod[c]['Magic']}")
    print(f"   vs Agility: +{best_mod[c]['Agility']}")
