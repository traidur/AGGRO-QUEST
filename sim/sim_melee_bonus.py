...
for cA in classes:
    for cB in classes:
        ...
        ev = MATRIX[cA][cB]
        # Melee gets +1 vs Ranged
        if is_melee(cA) and not is_melee(cB): ev += 1
        # Ranged vs Melee effectively loses 1 (since Melee got +1)
        if not is_melee(cA) and is_melee(cB): ev -= 1
        
        if ev > 0: wl[cA] += 1

print("--- WIN RATES (WITH 'MELEE GETS +1 VS RANGED' RULE) ---")
