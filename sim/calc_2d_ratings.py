...
# Hill climbing to find best assignment
for _ in range(200):
    current = {c: {a: random.choice([-2, -1, 0, 1, 2]) for a in archetypes} for c in classes}
        ...
        for c in classes:
            for a in archetypes:
                for step in [-1, 1]:
                    ...
                    neighbor[c][a] += step
                    if neighbor[c][a] < -2 or neighbor[c][a] > 2: continue
