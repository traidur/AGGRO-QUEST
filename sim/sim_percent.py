...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    # % of HP scoring
    pct_A = dmg_done_by_A / max_hp_B
    pct_B = dmg_done_by_B / max_hp_A
    return pct_A - pct_B
    ...
    for cB in class_names:
        ...
        for sA_list in seqs_A:
            for sB_list in seqs_B:
                ...
                if ev > 0.01: totals[cA]["W"] += 1
                elif ev < -0.01: totals[cA]["L"] += 1
                else: totals[cA]["T"] += 1

print("--- ACTUAL HAND-VS-HAND WINS (SCORE = % of HP DEALT, NO BUFFS) ---")
