...
# Create a custom Warrior resolve round that ignores the Execute 50% rule
def warrior_resolve_round_unlocked(state, card_name, stance, round_num, mob_pattern, hero_max_hp, hero_hp, mob_hp_remaining, mob_hp_total):
    ...
    if card.get("killing_blow") and (mob_hp_remaining - dmg_dealt) <= 0:
        dmg_taken = 0.0
    elif card.get("grants_range") and mob_type == "melee":
        dmg_taken = 0.0
    else:
...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    # ORIGINAL RULE: Raw Damage Dealt
...
print('--- WARRIOR (Unlocked Execute) vs CLASSES ---')
for c in CLASSES.keys():
    if c != "Warrior":
        print(f"vs {c}: {evaluate_matchup('Warrior', c):+.2f}")
