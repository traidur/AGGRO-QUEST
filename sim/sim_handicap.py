...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    # Baseline Win Condition (Raw Damage Dealt)
...
for cA in class_names:
    ...
    for cB in class_names:
        if cA != cB:
            results[cA].append(evaluate_matchup(cA, cB))

print('--- HANDICAP CALCULATION ---')
for c, evs in results.items():
    evs.sort(reverse=True)
    # We want 4 wins and 4 losses. So the 4th value should become positive, the 5th should remain negative.
    # The handicap required is the negative of the midpoint between the 4th and 5th values.
    # Or just print the exact values.
    print(f"{c} EVs: {[round(e, 2) for e in evs]}")
    h4 = -evs[3]
    h5 = -evs[4]
    print(f"  -> Ideal Handicap to go 4-4 is between {h4:+.2f} and {h5:+.2f}")
