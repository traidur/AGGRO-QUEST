...
MODS = {
    'Warrior': {'Martial': 1, 'Magic': 3, 'Agility': 0},
    'Cleric': {'Martial': -1, 'Magic': 0, 'Agility': -1},
    'Wizard': {'Martial': 0, 'Magic': 0, 'Agility': 0},
    'Paladin': {'Martial': 0, 'Magic': 0, 'Agility': 0},
    'Rogue': {'Martial': 2, 'Magic': 0, 'Agility': 0},
    'Ranger': {'Martial': 0, 'Magic': 0, 'Agility': 0},
    'Runecaster': {'Martial': 0, 'Magic': 0, 'Agility': 2},
    'Druid': {'Martial': 0, 'Magic': 0, 'Agility': -2},
    'Necromancer': {'Martial': 0, 'Magic': 0, 'Agility': 3}
}
ARCH_MAP = {
...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    # APPLY MODIFIERS
    mod_A = MODS[class_A][ARCH_MAP[class_B]]
    mod_B = MODS[class_B][ARCH_MAP[class_A]]
    ...
    for cB in class_names:
        ...
        for sA_list in seqs_A:
            for sB_list in seqs_B:
                ...
                total += 1
                if ev > 0.05: wins += 1
                elif ev >= -0.05: wins += 0.5 # Count ties as half win for WR calculation
