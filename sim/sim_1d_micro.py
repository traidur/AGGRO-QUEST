...
def resolve_duel(class_A, seq_A, class_B, seq_B):
    ...
    mod_A = MODS_1D[class_A]
    mod_B = MODS_1D[class_B]
...
for cA in class_names:
    for cB in class_names:
        ...
        wr = (wins / total) * 100
        if 25.0 <= wr <= 75.0: fair_count += 1
        total_count += 1
        print(f"{cA} vs {cB}: {wr:.1f}%")

print(f"\n1D SYSTEM FAIR MATCHUPS (25% to 75% Win Rate): {fair_count} out of {total_count}")
