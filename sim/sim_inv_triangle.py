...
for cA in classes:
    for cB in classes:
        ...
        # Inverted Triangle (Fixes our specific game math)
        if aA == 'Martial' and aB == 'Magic': ev += 2
        elif aA == 'Magic' and aB == 'Agility': ev += 2
        elif aA == 'Agility' and aB == 'Martial': ev += 2
        
        if aA == 'Magic' and aB == 'Martial': ev -= 2
        elif aA == 'Agility' and aB == 'Magic': ev -= 2
        elif aA == 'Martial' and aB == 'Agility': ev -= 2
        
        if ev > 0: wl[cA] += 1

print("--- WIN RATES (WITH INVERTED TRIANGLE, +2 Advantage) ---")
